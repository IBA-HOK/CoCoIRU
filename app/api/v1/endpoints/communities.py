...
@router.get("/", response_model=List[schemas.Communities])
def api_read_communities(
    skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
    return crud.get_communities(db, skip=skip, limit=limit)

@router.get("/{item_id}", response_model=schemas.Communities)
...
@router.put("/{item_id}", response_model=schemas.Communities)
...
@router.delete("/{item_id}", response_model=schemas.Communities)
