...
@router.post("/", response_model=schemas.SupportRequest)
...
@router.get("/", response_model=List[schemas.SupportRequest])
def api_read_support_requests(
    skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
    return crud.get_support_requests(db, skip=skip, limit=limit)

@router.get("/{item_id}", response_model=schemas.SupportRequest)
...
@router.put("/{item_id}", response_model=schemas.SupportRequest)
...
@router.delete("/{item_id}", response_model=schemas.SupportRequest)
